...
tile(Monitor *m)
{
	...
	if (n == 1) {
		ms = f * (m->ww - g), ss = m->ww - ms - g, ma->x = 0, ma->y = 0,
		ma->fx = ma->x + ms, ma->fy = m->wh, sa->x = ms + g, sa->y = 0,
		sa->fx = sa->x + ss, sa->fy = m->wh;
	} else {
        if(ga->dir == DirHor)
               	ms = f * (m->ww - g), ss = m->ww - ms - g,
               	ma->x = m->gappx, ma->y = m->gappx, ma->fx = ma->x + ms, ma->fy= m->wh - m->gappx,
               	sa->x = ms + g + m->gappx, sa->y = m->gappx, sa->fx = sa->x + ss - 2* m->gappx, sa->fy = m->wh -  m->gappx;
	else if(ga->dir == DirRotHor)
		ms = f * (m->ww - g), ss = m->ww - ms - g,
               	ma->x = ss + g + m->gappx, ma->y = m->gappx, ma->fx = ma->x + ms - 2*m->gappx, ma->fy = m->wh - m->gappx,
               	sa->x = m->gappx, sa->y = m->gappx, sa->fx = sa->x + ss, sa-> fy = m->wh - m->gappx;
	else if(ga->dir == DirVer)
		ms = f * (m->wh - g), ss = m->wh - ms - g,
                ma->x = m->gappx, ma->y = m->gappx, ma->fx = m->ww - m->gappx, ma->fy = ma->y + ms,
                sa->x = m->gappx, sa->y = ms + g + m->gappx, sa->fx = m->ww - m->gappx, sa->fy = sa->y + ss - 2*m->gappx;
	else
		ms = f * (m->wh - g), ss = m->wh - ms - g,
                ma->x = m->gappx, ma->y = ss + g + m->gappx, ma->fx = m->ww - m->gappx, ma->fy = ma->y + ms - 2*m->gappx,
                sa->x = m->gappx, sa->y = m->gappx, sa->fx = m->ww - m->gappx, sa->fy = sa->y + ss;
	}
        /* tile clients */
        for(c = nexttiled(m->clients), i = 0; i < n; c = nexttiled(c->next), i++) {
		a = ma->n > 0 ? ma : sa;
		f = i == 0 || ma->n == 0 ? a->fact : 1, f /= --a->n + f;
                w = a->dir == DirVer ? a->fx - a->x : f * (a->fx - a->x - a->n * m->gappx);
                h = a->dir == DirHor ? a->fy - a->y : f * (a->fy - a->y - a->n * m->gappx);
...
updatesystray(void)
{
	...
	XFillRectangle(dpy, systray->win, /*drw->gc*/XCreateGC(dpy, root, 0 ,
NULL) , 0, 0, w, bh);
	XSync(dpy, False);
}
